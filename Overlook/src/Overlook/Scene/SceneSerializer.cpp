...
namespace YAML {
	...
	template<>
	struct convert<Overlook::UUID>
	{
		static Node encode(const Overlook::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Overlook::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	{
		OL_CORE_ASSERT(entity.HasComponent<IDComponent>());
...
// 						WRITE_SCRIPT_FIELD(Entity, UUID);
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		OL_CORE_TRACE("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				OL_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
				...
				if (scriptComponent)
				{
					...
					if (scriptFields)
					{
						...
						for (auto scriptField : scriptFields)
						{
							...
							// TODO(Yan): turn this assert into Overlooknut log warning
							OL_CORE_ASSERT(fields.find(name) != fields.end());
