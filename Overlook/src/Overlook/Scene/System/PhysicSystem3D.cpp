...
#include "Overlook/Utils/MathUtils.h"
...
namespace Overlook
{
	...
	void PhysicSystem3D::OnRuntimeStart()
	{
		...
		mDynamicsWorld->setGravity(btVector3(0.0f, -0.98f, 0.0f));
		//mDynamicsWorld->setForceUpdateAllAabbs(true);

		auto view = mScene->m_Registry.view<TransformComponent, Rigidbody3DComponent>();
		for (auto e : view)
		{
			...
			{
				// Calculate Obb
				OL_CORE_ASSERT(entity.HasComponent<ModelRendererComponent>());
				auto& transform = entity.GetComponent<TransformComponent>();
			...
			{
				btScalar radius = transform.Scale.x;
			...
			{
				body->setCollisionFlags(body->getCollisionFlags() | btCollisionObject::CollisionFlags::CF_DYNAMIC_OBJECT);
			}
	...
	void PhysicSystem3D::OnUpdateRuntime(Timestep ts)
	{
		...
		{
			Entity camera = mScene->GetPrimaryCameraEntity();
	...
	{
		//auto view = mScene->m_Registry.view<Rigidbody3DComponent>();
		//for (auto e : view)
		//{
		//	Entity entity = { e, mScene };
		//	auto& rb3d = entity.GetComponent<Rigidbody3DComponent>();
