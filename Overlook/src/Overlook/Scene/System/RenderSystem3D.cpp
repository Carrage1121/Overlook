...
namespace Overlook
{
	...
	void RenderSystem3D::OnUpdateRuntime(Timestep ts)
	{
		...
		if (mainCamera)
		{
			...
			// Draw model
			{
				auto view = mScene->m_Registry.view<TransformComponent, ModelRendererComponent>();
				for (auto entity : view)
				{
					auto [transform, mesh] = view.get<TransformComponent, ModelRendererComponent>(entity);
	...
	void RenderSystem3D::OnUpdateEditor(Timestep ts, EditorCamera& camera)
	{
		...
		// Directional light
		{
			auto view = mScene->m_Registry.view<TransformComponent, DirectionalLightComponent>();
